...
public class FraudDetectionService {
    ...
    public boolean isFraud(TransactionData transaction, float threshold) throws Exception {
        float probability = predictAsync(transaction).get();
    ...
    private float[] preprocess(TransactionData transaction) {
        ...
        return new float[]{
            ...
            (float) transaction.getAvgUserAmount(),
            (float) transaction.getTxFrequency(),
            (float) transaction.getHourSin(),
            (float) transaction.getHourCos(),
            (float) transaction.getDayOfWeek(),
            (float) transaction.getIpRiskScore()
        };
    }
    
    public void updateModel(String version) throws Exception {
        loadModel(version);
    }
    ...
    public static DatasetPair generateSyntheticDataset(int numSamples, Random random) {
        ...
        for (int i = 0; i < numSamples; i++) {
            ...
            if (isFraud) {
                tx.setAmount(random.nextFloat() * 0.8 + 0.7);
                tx.setAvgUserAmount(random.nextFloat() * 0.3);
                tx.setTxFrequency(random.nextFloat() * 0.3);
                tx.setHourSin(random.nextFloat() * 0.5 - 0.25);
                tx.setHourCos(random.nextFloat() * 0.5 - 0.25);
                tx.setDayOfWeek(random.nextFloat());
                tx.setIpRiskScore(random.nextFloat() * 0.5 + 0.5);
            } else {
                tx.setAmount(random.nextFloat() * 0.5 + 0.1);
                tx.setAvgUserAmount(random.nextFloat() * 0.5 + 0.2);
                tx.setTxFrequency(random.nextFloat() * 0.5 + 0.3);
                tx.setHourSin(random.nextFloat() * 2 - 1);
                tx.setHourCos(random.nextFloat() * 2 - 1);
                tx.setDayOfWeek(random.nextFloat());
    ...
    public static void main(String[] args) throws Exception {
        ...
        normalTx.setAmount(0.3);
        normalTx.setAvgUserAmount(0.35);
        normalTx.setTxFrequency(0.6);
        normalTx.setHourSin(0.5);
        normalTx.setHourCos(0.8);
        ...
        fraudTx.setAmount(1.2);
        fraudTx.setAvgUserAmount(0.2);
        fraudTx.setTxFrequency(0.1);
        fraudTx.setHourSin(-0.2);
        fraudTx.setHourCos(0.1);
        fraudTx.setDayOfWeek(0.8);
        fraudTx.setIpRiskScore(0.9);
